...
+ Live side-by-side preview window for demo / screen recording
"""
...
# Live preview for demo / screen recording
SHOW_LIVE_PREVIEW = True          # set False to disable the live window
PREVIEW_DELAY_MS = 1200           # how long each image stays on screen (ms)
...
def show_side_by_side(original_bgr, annotated_bgr, title: str, delay_ms: int = 1200):
    """
    Display original (LEFT) and detection result (RIGHT) in one window.
    Press 'q' or Esc to stop early.
    """
    h = max(original_bgr.shape[0], annotated_bgr.shape[0])
    w = max(original_bgr.shape[1], annotated_bgr.shape[1])

    def resize_to(img, target_h, target_w):
        return cv2.resize(img, (target_w, target_h))

    left = resize_to(original_bgr, h, w)      # ORIGINAL on the left
    right = resize_to(annotated_bgr, h, w)    # DETECTION on the right
    ...
    # header bar
    header_h = 40
    header = np.full((header_h, canvas.shape[1], 3), 30, dtype=np.uint8)
    cv2.putText(header, title, (12, 28), FONT, 0.7, (0, 255, 255), 2)
    canvas = np.vstack([header, canvas])

    # labels anchored to the upper-right of each panel with a black backing for contrast
    orange = (0, 140, 255)
    left_text = "ORIGINAL INPUT"
    right_text = "DETECTION OUTPUT"
    left_size, _ = cv2.getTextSize(left_text, FONT, 0.7, 2)
    right_size, _ = cv2.getTextSize(right_text, FONT, 0.7, 2)

    left_x = w - left_size[0] - 18
    right_x = w * 2 + 8 + 8 - right_size[0] - 18
    label_y = header_h + 26
    pad = 8

    left_rect = (left_x - pad, header_h + 8, left_size[0] + pad * 2, left_size[1] + pad)
    right_rect = (right_x - pad, header_h + 8, right_size[0] + pad * 2, right_size[1] + pad)

    cv2.rectangle(canvas, (left_rect[0], left_rect[1]), (left_rect[0] + left_rect[2], left_rect[1] + left_rect[3]), (0, 0, 0), -1)
    cv2.rectangle(canvas, (right_rect[0], right_rect[1]), (right_rect[0] + right_rect[2], right_rect[1] + right_rect[3]), (0, 0, 0), -1)

    cv2.putText(canvas, left_text, (left_x, label_y), FONT, 0.7, orange, 2, cv2.LINE_AA)
    cv2.putText(canvas, right_text, (right_x, label_y), FONT, 0.7, orange, 2, cv2.LINE_AA)

    cv2.namedWindow("AI-Sprayer Live Detection", cv2.WINDOW_NORMAL)
    cv2.imshow("AI-Sprayer Live Detection", canvas)
...
def detect_with_locateanything(image: Image.Image, model, processor, prompt: str):
    ...
    pure_nn_ms = (time.perf_counter() - model_start) * 1000.0
    ...
    return detections, raw_text, pure_nn_ms
...
def process_folder(input_dir: str, output_dir: str, model, processor=None):
    ...
    print(f"Live Preview: {SHOW_LIVE_PREVIEW}\n")
    ...
    for idx, img_path in enumerate(image_files):
        ...
        if MODEL_TYPE == "yolo":
            detections, raw_text, speed = detect_with_yolo(pil_image, model)
            pure_nn_ms = float(speed.get("inference_ms", 0.0))
        else:
            detections, raw_text, pure_nn_ms = detect_with_locateanything(pil_image, model, processor, prompt)
            speed = {}

        elapsed = time.time() - t_start
        e2e_ms = elapsed * 1000.0
        pure_nn_ms = float(pure_nn_ms) if pure_nn_ms is not None else 0.0
        print(f"  → {len(detections)} weed(s) detected | Pure NN: {pure_nn_ms:.1f} ms | End-to-End: {e2e_ms:.1f} ms")
        ...
        # ─── Live side-by-side preview (LEFT=original, RIGHT=detection) ───
        if SHOW_LIVE_PREVIEW:
            title = (
                f"Weed Counts: {len(detections)}  |  "
                f"Pure NN Latency: {pure_nn_ms:.1f} ms  |  "
                f"End-to-End Latency: {e2e_ms:.1f} ms"
            )
            keep_going = show_side_by_side(bgr_image, annotated, title, PREVIEW_DELAY_MS)
            if not keep_going:
